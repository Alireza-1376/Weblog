...
function Pagination({ allArticles, countPerPage }: { allArticles: Blog[], countPerPage: number }) {
    const searchParams = useSearchParams();
    const page = searchParams.get("page");
    const currentPage = Number(page) || 1;
    const lastPage = Math.ceil(allArticles.length / countPerPage);
    const startIndex = (currentPage - 1) * countPerPage;
    const currentItems = allArticles.slice(startIndex, startIndex + countPerPage);
